...
export default function Calendar(props: ICalendarProps) {
  ...
  const groupedCourtCases =
    calendarData.courtCases && calendarData.courtCases.length > 0 ? groupByTime(calendarData.courtCases) : {};

  return (
    <>
      <div className="input-group">
        <CustomDayPickerInput setDateParam={setDateParam} selectedDay={new Date(dateParam)}>
          <button className="btn btn-outline-secondary" type="button" onClick={() => setDate(dateParam)}>
            Search
          </button>
      ...
      <div className="container-fluid">
        {calendarData.courtCases && calendarData.courtCases.length > 0 && (
        ...
        {calendarData.courtCases &&
          ...
          Object.keys(groupedCourtCases).map((key: string, index: number) => (
            <CalendarRow
              key={key}
              time={key}
              courtCases={groupedCourtCases[key]}
              rowIndex={index}
              disableGridItem={disableGridItem}
  ...
  async function disableGridColumn(columnIndex: number) {
    try {
      const courtCasesToUpdate = calendarData.courtCases.filter(
        (courtCase, i: number) => i % columnCount === columnIndex,
      );
...
function CalendarRow(props: ICalendarRowProps): JSX.Element {
...
function GridColumnHeadings(props: { columnCount: number; disableGridColumn(columnCount: number): void }): JSX.Element {
    ...
    headings.push(
      <div className={`col ${styles.calendarHeaderItem}`} key={i}>
        ...
        <ConfirmDialog
          ...
          message={
            "This will disable all items in this grid column and no new court cases will be created in this column. Continue?"
          }
  ...
  return (
    <div className={`row ${styles.calendarHeaderContainer}`}>
      <div className={styles.calendarHeaderTimeColumn}>Time</div>
      {headings}
    </div>
  );
}
...
export function CalendarItem(props: IPropsGridItem) {
  const { fileNo, court, firstName, lastName, email, phoneNumber, registeredAt } = props.courtCase;
